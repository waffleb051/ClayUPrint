...
void TemperatureController::updateTempControlVars()
{
    ...
    {
        tempIStateLimitMax = 0; //(float)pidDriveMax*10.0f/pidIGain;
        tempIStateLimitMin = 0; //(float)pidDriveMin*10.0f/pidIGain;
    }
#endif
}
...
#ifdef SUPPORT_MAX6675
    case 101: // MAX6675
        currentTemperature = 0; // changed from read_max6675(sensorPin);
        break;
#endif
#ifdef SUPPORT_MAX31855
    case 102: // MAX31855
        currentTemperature = 0; //changed from read_max31855(sensorPin);
#endif
    default:
        currentTemperature = 0; //4095; // unknown method, return high value to switch heater off for safety
    }
...
void TemperatureController::autotunePID(float temp,uint8_t controllerId,bool storeValues)
{
    ...
    float currentTemp=0; //Might need to modiofy this variable, changed to 0 
    int cycles=0;
    bool heating = true;
    ...
    float Ku, Tu;
    float Kp, Ki, Kd;
    float maxTemp=20, minTemp=0;  //changed min from 20 to 0
    ...
    for(;;)
    {
        ...
        if(heating == true && currentTemp > temp)   // switch heating -> off
        {
            ...
            {
                heating=true; //changed from false to true
